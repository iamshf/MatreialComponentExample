...
class ComplexDataStore {
  ...
  getChildren(parentId: string) {
    ...
    return this.select(this._allData, this._loadingData, (data, loading) => {
      ...
      console.log(data, parentData, parentId);
...
@Component({
  ...
  standalone: true,
  imports: [MatTreeModule, MatIcon, MatProgressSpinnerModule, CommonModule],
  templateUrl: './mattree.component.html',
  styleUrl: './mattree.component.scss'
})
export class MattreeComponent {
  ...
  trackBy = (index: number, node: TransformedData) => node.raw.id;
  expansionKey = (node: TransformedData) => node.raw.id;

  ngOnInit() {
    this._dataStore.loadRoots();
    // console.log(this.roots.subscribe(value => console.log(value)));
  }

  onExpand(node: TransformedData, expanded: boolean) {
    // (this.roots.subscribe(value => console.log(value)))
    if (expanded) {
  ...
  hasChild = (_: number, node: TransformedData) => node.isExpandable()
}
